...
def filter_cases(data, stops, tofit):
    ...
    if model_para[8] == 'YES':
        ...
        total_cons, inputs = Bioaccum.bio_monte_carlo_loop(model_para, all_data, 0, time_per_step ,fish_by_region, prior_locations, locations , u_iter, v_iter)

        if stat_check == True:
            total_cons = pr.make_result_dist(total_cons, tofit)
        ...
        while t < time_steps:
            ...
            uv_single, inputs = Bioaccum.bio_monte_carlo_loop(model_para, all_data, t, time_per_step, fish_by_region, prior_locations, locations, u_iter, v_iter, p_dic=total_cons)
...
def steady_state_output(total_cons, stat_check, output_name, dist_type, inputs, data, tofit):
    if stat_check == False:
        total_cons = total_cons[0]
    FR_Input_Output.write_output_steady(total_cons, output_name, 'Steady State Simulation', dist_type, inputs, data, tofit)

def temporal_output(stat_check, to_write, output_name, stops, region_areas, dist_type, inputs, data, tofit):

        #Non-statistical
        if stat_check == False:
            FR_Input_Output.write_temporal_excel(to_write, output_name, stops, 0, region_areas, dist_type, inputs, data, tofit)
        #statistical
        else:
            FR_Input_Output.write_temporal_excel(to_write, output_name, stops, 1, region_areas, dist_type, inputs, data, tofit)
